...
export default function Unit7() {
  const unitData = unitsdata['7'];
  const [selected, setSelected] = useState(0);
  const subunit = unitData[selected];
  const unitName = unitData[0]?.subunit?.split(':')[0]?.replace(/Unit 7\.?/, '').trim() || '';
        ...
        {unitData.map((s, idx) => (
          <button
            ...
            style={{
              ...
              height: 36,
              borderRadius: '50%',
              background: selected === idx ? '#ffedd5' : '#eee', // orange-100
              color: selected === idx ? '#b45309' : '#222', // orange-700
              border: 'none',
              margin: '0 8px',
              fontWeight: 'bold',
              fontSize: 16,
