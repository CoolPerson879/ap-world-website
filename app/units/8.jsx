...
export default function Unit8() {
  const unitData = unitsdata['8'];
  const [selected, setSelected] = useState(0);
  const subunit = unitData[selected];
  const unitName = unitData[0]?.subunit?.split(':')[0]?.replace(/Unit 8\.?/, '').trim() || '';

  return (
  <div className="bg-stone-200" style={{ padding: 24, minHeight: '100vh' }}>
      <div style={{ position: 'relative', marginBottom: 32 }}>
        ...
        <span style={{ display: 'block', textAlign: 'center', fontSize: 44, fontWeight: 700, letterSpacing: -1, marginBottom: 0, color: '#222' }}>
          Unit 8
          <span style={{ fontSize: 24, fontWeight: 400, marginLeft: 16, color: '#444' }}>{unitName && `- ${unitName}`}</span>
        </span>
      </div>
      ...
      <div style={{ margin: '0 auto', maxWidth: 700, background: '#fff', borderRadius: 12, padding: 32, boxShadow: '0 2px 8px #eee' }}>
          ...
          {subunit.events.map((event, i) => (
            event.subheading ? (
              <li key={i} style={{ fontWeight: 'bold', marginTop: 18, textAlign: 'center', fontSize: 18 }}>{event.subheading}</li>
                ...
                >
                <span style={{ fontSize: 18, marginRight: 10, marginTop: 2 }}>•</span>
