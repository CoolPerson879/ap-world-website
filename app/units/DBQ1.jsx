...
import { DBQ1, DBQQuestions } from '../FRQData.js';
import React, { useState } from 'react';
import Link from 'next/link';

export default function DBQ1Page() {
  const unitData = DBQ1;
  const [selected, setSelected] = useState(0);
  const subunit = unitData[selected];

  // Get the question for DBQ1 (id: 1)
  const question = DBQQuestions.find(q => q.id === 1)?.question || '';
  ...
  return (
    <div className="bg-stone-200" style={{ padding: 24, minHeight: '100vh', fontWeight: 300 }}>
      ...
      {/* Question Container - below the single button */}
      <div style={{ margin: '0 auto', maxWidth: 700, background: '#fff', borderRadius: 12, padding: 32, boxShadow: '0 2px 8px #eee', fontWeight: 400, marginBottom: 32 }}>
        <h3 style={{ textAlign: 'center', fontSize: 22, marginBottom: 0, color: '#b45309', fontWeight: 700 }}>DBQ 1 Question</h3>
